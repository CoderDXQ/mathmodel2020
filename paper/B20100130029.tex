...
\begin{equation}\label{EIlxgx}
	EI_{y^*}(x) \to (\gamma+\frac{g(x)}{l(x)}(1 - \gamma))^{-1}
\end{equation}
...
用之前得到的硫预测模型作为评价函数，以与硫含量相关的8个主要变量作为待优化变量，并根据操作变量的取值范围得到一个参数空间$\mathbb{H}_S \in \mathbb{R}^8$，用贝叶斯优化库\texttt{HyperOpt}的TPE算法对工业数据的325个样本逐一进行优化。
